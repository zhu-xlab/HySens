...
def load_berlin_wavelengths():
    vnir = np.linspace(420, 1000, 92, endpoint=True)
    swir = np.linspace(900, 2450, 152, endpoint=True)
    hs_wavelength_berlin = np.concatenate([vnir, swir]).astype(np.float32)
    return hs_wavelength_berlin
...
class BerlinDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        min_vals = patch_hs.min(axis=-1, keepdims=True)
        max_vals = patch_hs.max(axis=-1, keepdims=True)
        ...
        normalized_patch_hs = torch.tensor(normalized_patch_hs, dtype=torch.float32)
...
def create_data_loaders_berlin(dataset, batch_size):
