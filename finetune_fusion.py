...
from models.hysens_model import HysensFusionModel
...
class Config:
    ...
    # dataset switch
    # -----------------------
    dataset = "augsburg"  # augsburg / berlin
    ...
    dataset_map = {
        "augsburg": {
            "data_hs_dir": "HS-SAR-DSM Augsburg/data_HS_LR.mat",
            "data_sar_dir": "HS-SAR-DSM Augsburg/data_SAR_HR.mat",
            "label_train_dir": "HS-SAR-DSM Augsburg/TrainImage.mat",
            "label_test_dir": "HS-SAR-DSM Augsburg/TestImage.mat",
        },
        "berlin": {
            "data_hs_dir": "HS-SAR Berlin/data_HS_LR.mat",
            "data_sar_dir": "HS-SAR Berlin/data_SAR_HR.mat",
            "label_train_dir": "HS-SAR Berlin/TrainImage.mat",
            "label_test_dir": "HS-SAR Berlin/TestImage.mat",
        }
    }
    ...
    def get_paths(cls):
        ...
        return (
            os.path.join(cls.data_root, cfg["data_hs_dir"]),
            os.path.join(cls.data_root, cfg["data_sar_dir"]),
            os.path.join(cls.data_root, cfg["label_train_dir"]),
            os.path.join(cls.data_root, cfg["label_test_dir"]),
        )
...
def train(model, train_loader, criterion, optimizer, device):
    ...
    for batch in train_loader:
        hyper_patch, sar_patch, labels = batch
        hyper_patch, sar_patch, labels = hyper_patch.to(device), sar_patch.to(device), labels.to(device)

        optimizer.zero_grad()

        outputs = model(hyper_patch, sar_patch)
...
def main_training_loop(model, train_loader, val_loader, test_loader,
    ...
    # Initialize variables to track the best validation AA and corresponding epoch
    best_val_oa = -np.inf
    best_val_aa = -np.inf
    best_epoch = -1
...
def run_experiments(num_runs: int, base_seed: int = 42, output_file: str = "results.csv"):
    ...
    for i in range(num_runs):
        ...
        # single run
        cfg = Config()
        data_hs_dir, data_sar_dir, label_train_dir, label_test_dir = cfg.get_paths()

        # -----------------------
        # WAVELENGTHS
        # -----------------------
        dataset_fusion = get_dataset(cfg.dataset)
        ...
        # DATASET
        # -----------------------
        dataset = dataset_fusion(data_hs_dir=data_hs_dir, data_sar_dir=data_sar_dir,
                                 label_train_dir=label_train_dir, label_test_dir=label_test_dir,
        ...
        hidden_dim = 96
        ref_num_blocks = 70
        input_dim = dataset.image_hs.shape[2]
        ...
        model = HysensFusionModel(encoder=encoder, aggregator=aggregator,
